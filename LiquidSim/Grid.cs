...
namespace LiquidSim
{
    ...
    public sealed class Grid
    {
        ...
        /// <summary>
        /// Each cell containing air belongs to an air volume; each air volume
        /// has a total mass.
        /// </summary>
        ...
        /// <summary>
        /// Indicates which cells have something other than liquid/air in.
        /// </summary>
        private CellSolidKind[,] solid;
        private readonly float[,] solidU, solidV;
        ...
        public Grid(int xSize, int ySize)
        {
            ...
            solidU = new float[xSize, ySize];
            solidV = new float[xSize, ySize];
        ...
        public float InitialAirPressure { get; set; }
        ...
        public void SetSolidObject(int x, int y, float u, float v)
        {
            solid[x, y] = CellSolidKind.Object;
            solidU[x, y] = u;
            solidV[x, y] = v;
        }

        public void SetSolid(int x, int y, CellSolidKind kind = CellSolidKind.Fixed)
        {
            solid[x, y] = kind;
            solidU[x, y] = 0;
            solidV[x, y] = 0;
        ...
        private void ApplyVelocityBoundaryCondition(bool zeroBoundaries)
        {
            for (int x = 0; x < XSize; x++)
            {
                if (zeroBoundaries)
                ...
                for (int y = 1; y < YSize; y++)
                {
                    if ((cellFlags[x, y - 1] & (uint)CellVolumeFlags.HasBottomBoundary) != 0)
                    {
                        ...
                        {
                            v[x, y] = Math.Min(v[x, y], solidV[x, y]);
                        }
                    }
                    else if ((cellFlags[x, y] & (uint)CellVolumeFlags.HasTopBoundary) != 0)
                    {
                        ...
                        {
                            v[x, y] = Math.Max(v[x, y], solidV[x, y - 1]);
                        }
                    }
                    else
                    {
            ...
            for (int y = 0; y < YSize; y++)
            {
                ...
                for (int x = 1; x < XSize; x++)
                {
                    if ((cellFlags[x - 1, y] & (uint)CellVolumeFlags.HasRightBoundary) != 0)
                    {
                        ...
                        {
                            u[x, y] = Math.Min(u[x, y], solidU[x, y]);
                        }
                    }
                    else if ((cellFlags[x, y] & (uint)CellVolumeFlags.HasLeftBoundary) != 0)
                    {
                        ...
                        {
                            u[x, y] = Math.Max(u[x, y], solidU[x - 1, y]);
                        }
                    }
                    else
                    {
        ...
        private void CalculateInitialAirMasses()
        {
            DiscoverAirVolumes();
            SwapAirVariables();

            airVolumeMasses.Clear();
            foreach (int cellCount in airVolumeCellCounts)
            {
                airVolumeMasses.Add(InitialAirPressure * cellCount);
            }
        }

        private void UpdateAirVolumes()
        {
            DiscoverAirVolumes();

            tempAirVolumeMap.Clear();
            for (int x = 0; x < XSize; x++)
            {
                for (int y = 0; y < YSize; y++)
                {
                    int newVol = tempAirVolume[x, y];
                    if (newVol >= 0)
                    {
                        int oldVol = airVolume[x, y];
                        if (oldVol >= 0)
                        {
                            tempAirVolumeMap.Add((oldVol, newVol));
                        }
                    }
                }
            }

            for (int i = 0; i < airVolumeCellCounts.Count; i++)
            {
                airVolumeCellCounts[i] = 0;
            }

            foreach (var (oldVol, newVol) in tempAirVolumeMap)
            {
                airVolumeCellCounts[oldVol] += tempAirVolumeCellCounts[newVol];
            }

            tempAirVolumeMasses.Clear();
            for (int i = 0; i < tempAirVolumeCellCounts.Count; i++)
            {
                tempAirVolumeMasses.Add(0);
            }

            foreach (var (oldVol, newVol) in tempAirVolumeMap)
            {
                tempAirVolumeMasses[newVol] += airVolumeMasses[oldVol] * tempAirVolumeCellCounts[newVol] / airVolumeCellCounts[oldVol];
            }

            SwapAirVariables();
        }

        private void SwapAirVariables()
        {
            (airVolume, tempAirVolume) = (tempAirVolume, airVolume);
            (airVolumeCellCounts, tempAirVolumeCellCounts) = (tempAirVolumeCellCounts, airVolumeCellCounts);
            (airVolumeMasses, tempAirVolumeMasses) = (tempAirVolumeMasses, airVolumeMasses);
        }
